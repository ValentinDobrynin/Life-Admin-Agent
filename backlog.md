...
**Статус:** ✅ Done
**Приоритет:** High
**Компонент:** `models.py`, `modules/reference.py`, `bot/handlers.py`, `migrations/versions/6c9d2e3f4a5b_r2_key_to_r2_keys_in_reference_data.py`, `tests/test_reference_file.py`
...
- Миграция: добавить колонку `r2_keys`, конвертировать `r2_key → [r2_key]`, удалить `r2_key`
- Новый паттерн caption: `справочник #<id>` (без двоеточия) — добавить файл к существующей записи
- `extract_reference_append_id(caption) -> int | None` — новая функция в `modules/reference.py`
- `append_file_to_reference(ref_id, file_bytes, filename, db)` — загружает в R2, добавляет ключ в `r2_keys`
- При отправке файлов: итерировать `r2_keys`, caption только на первый файл
- После создания записи — hint пользователю: «Есть ещё страницы? Отправь с подписью: справочник #<id>»
- `get_ref_card_text`: суффикс `📎×N` если несколько файлов у owned items

**Acceptance Criteria**
- [x] `reference_data.r2_keys` — JSON array, миграция применена, существующие данные сконвертированы
- [x] Caption `справочник #11` добавляет файл к записи #11 без создания новой записи
- [x] Caption `справочник: водительские права` создаёт новую запись как раньше
- [x] `пришли права` — отправляет все прикреплённые файлы по порядку
- [x] Существующие записи с одним файлом продолжают работать корректно
- [x] Тесты: `extract_reference_append_id` (valid/invalid), `append_file_to_reference` (found/not found), `parse_and_save` stores r2_keys as list
- [x] `make check` проходит

**Resolution**
- `r2_key` удалён из `ReferenceData`, добавлен `r2_keys: list[str]` (JSON, default `[]`)
- Миграция `6c9d2e3f4a5b`: `json_build_array(r2_key)` для существующих данных, downgrade восстанавливает `r2_key` из первого элемента
- `extract_reference_append_id` и `append_file_to_reference` добавлены в `modules/reference.py`
- `_handle_photo` и `_handle_document` в `bot/handlers.py`: проверяют append-паттерн первым, при создании записи добавляют hint
- При отправке файлов по запросу: caption только на первый файл в цикле
- 8 новых тестов + 3 обновлённых; 189 тестов — все зелёные

**Связанные задачи**
- UX-001 (Этап 2): кнопка «📎 Добавить ещё страницу» через state machine
- UX-006: отправка всех фото одним медиа-альбомом
- UX-007: приём нескольких фото одним сообщением от пользователя

---

### [FEATURE-016] Генерация чеклиста вещей для поездки

**Статус:** ✅ Done
**Приоритет:** High
**Компонент:** `prompts/trip_checklist.txt`, `modules/parser.py`, `modules/suggestions.py`, `bot/handlers.py`, `scheduler.py`, `tests/test_suggestions.py`, `tests/test_parser.py`

**Описание проблемы**
Пользователь вручную составляет список вещей перед каждой поездкой. Агент знает о поездке (entity type=trip), но никак не помогает с подготовкой. Чеклист поездки часто пустой.

**Ожидаемое поведение**
- Пользователь пишет «Собери чеклист вещей для поездки в Дубай 20-25 апреля» → агент получает погоду (Open-Meteo) и особенности направления (DuckDuckGo), генерирует список 15-30 пунктов через OpenAI, сохраняет в `checklist_items` entity если найдена
- При первом напоминании о поездке без чеклиста — чеклист генерируется автоматически

**Технические детали**
- `detect_intent` → новый intent `checklist_trip`
- `parse_trip_checklist_request(text)` в `modules/parser.py` — извлекает destination, dates, trip_type, travelers
- `_fetch_trip_context(destination, dates)` в `modules/suggestions.py` — httpx GET к Open-Meteo (геокодирование + погода) и DuckDuckGo Instant Answer API
- `generate_trip_checklist(trip_params, db)` — генерирует чеклист, сохраняет с префиксом `[авто]`, возвращает `(list[str], Entity | None)`
- `ensure_trip_checklist(entity, db)` — проактивно вызывается из `check_reminders_job` перед `enrich_reminder`
- Кнопка `✈️ Создать поездку` (callback `create_trip_`) → временный ответ «Напиши боту: Поездка в {destination}»

**Acceptance Criteria**
- [x] `detect_intent("собери чеклист для поездки в Дубай")` → `"checklist_trip"`
- [x] `parse_trip_checklist_request` возвращает destination, dates, trip_type, travelers
- [x] `generate_trip_checklist` вызывает Open-Meteo и DuckDuckGo
- [x] Entity найдена → пункты в `checklist_items` с префиксом `[авто]`
- [x] Entity не найдена → кнопка `✈️ Создать поездку`
- [x] `ensure_trip_checklist` генерирует чеклист при первом напоминании о поездке
- [x] Направление не указано → бот просит уточнить
- [x] `make check` проходит

**Resolution**
- Новый intent `checklist_trip` добавлен в `detect_intent` (проверяется до `entity`)
- `parse_trip_checklist_request` в `modules/parser.py` — OpenAI извлекает destination/dates/trip_type/travelers
- `_fetch_trip_context` в `modules/suggestions.py` — httpx: геокодирование через Open-Meteo geocoding API, погода через Open-Meteo forecast API (WMO codes → читаемый текст), особенности через DuckDuckGo Instant Answer API; все ошибки обрабатываются gracefully
- `generate_trip_checklist` — удаляет старые `[авто]` пункты перед пересохранением
- `ensure_trip_checklist` — вызывается из `check_reminders_job` в `scheduler.py` перед `enrich_reminder`, только для `entity.type == "trip"`
- `create_trip_` callback — временный ответ с инструкцией; полный handler в UX-010
- 11 новых тестов (7 parser + 4 suggestions); 200 тестов — все зелёные

**Связанные задачи**
- UX-008: FSM — уточнить матчинг поездки (после UX-001)
- UX-009: FSM — уточнить тип поездки и состав (после UX-001)
- FEATURE-017: Мёрж двух билетов в одну поездку
...
**Ожидаемое поведение**
При загрузке брони отеля — проверить есть ли trip entity с пересекающимися датами в том же направлении. Если да — предложить объединить. Если нет — создать новый trip.
...
| — | UX-006: Отправка нескольких файлов медиа-альбомом | 🆕 To Do |
| — | UX-007: Приём нескольких фото одним сообщением от пользователя | 🆕 To Do |
