...
async def generate_text(user_request: str, db: AsyncSession) -> str:
    ...
    system_prompt = (
        ...
        "1. Если пользователь упомянул марку, бренд, тип или название "
        "(Toyota, BMW, загранпаспорт, паспорт РФ, страховка и т.п.) — "
        "найди ЛУЧШЕЕ совпадение в справочнике и используй его без вопросов, "
        "даже если других объектов того же типа несколько.\n"
        "2. Если нужного типа объект только один — используй без вопросов.\n"
        "3. Уточняющий вопрос задаётся ТОЛЬКО если пользователь написал обобщённо "
